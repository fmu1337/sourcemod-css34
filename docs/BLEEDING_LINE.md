...
- `apply-sourcemod-v112.sh` — restores `g_SHPtr` extern + `MetaFactory(MMIFACE_SOURCEHOOK)` init in `smsdk_ext` when MM 1467+ `PLUGIN_GLOBALVARS`/`PLUGIN_SAVEVARS` drop SourceHook
